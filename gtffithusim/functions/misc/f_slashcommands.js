...
for (var i = 0; i < keys.length; i++) {
  ...
  if (c["name"] == "garage") {
    c = gte_SLASHCOMMANDS.garageoptions()
  }
  if (c["name"] == "car") {
    c["options"] = gte_SLASHCOMMANDS.caroptions()
  }
  if (c["name"] == "wheels") {
    c["options"] = gte_SLASHCOMMANDS.wheeloptions()
  }
  if (c["name"] == "home") {
    c["options"] = gte_SLASHCOMMANDS.homeoptions()
  }
...
module.exports.garageoptions = function () {
  var options = gte_SLASHCOMMANDS.caroptions().slice(1)
  var garage = {
      name: "garage",
