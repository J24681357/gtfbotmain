        ...
        gtfbot["rolelimit"] = true        
         timer = setInterval(function() {
            ...
            if (gtfbot["totalrole"] != 0) {
              gtfbot["totalrole"]--
              if (type == "ADD") {
              //gte_DISCORD.reply(msg, {content: "✅ " + "**" + role.name + "** role" + " " + "added."})             
              user.roles.add(role).catch(console.error);
              } else if (type == "REMOVE") {
              //   gte_DISCORD.reply(msg, {content: "✅ " + "**" + role.name + "** role" + " " + "removed."}) 
...
module.exports.autoMessage = function(client, title, text, color, image, channelid, elist, number) {
 ...
 function next(elist) {
    ...
    channel.messages.fetch(arr[number - 1][0]).then(msg => {
      ...
      embed.setDescription(description);
      if (elist.length != 0) {
        var buttons = gte_TOOLS.prepareButtons(elist, msg, {id:"ALL", garage: [], settings: gte_defaultsettings});
      } else {
        var buttons = []
      }
    ...
    var role = msg.guild.roles.cache.find(r => r.name === elist[val]["value"]);
                    ...
                    */
                    ...
                    if (useri.roles.cache.find(r => r.name === elist[val]["value"])) {
                      gte_DISCORD.role(msg, useri, role, "REMOVE")
                    } else {
                      gte_DISCORD.role(msg, useri, role, "ADD")
                    }
         })
         }
        if (elist.length != 0) {
        gte_TOOLS.createButtons(buttons, elist, functionlist, msg, {id:"ALL", garage: [], settings: gte_defaultsettings})
      }
