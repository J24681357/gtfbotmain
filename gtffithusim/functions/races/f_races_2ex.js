...
module.exports.raceLengthCalc = function(racesettings, racedetails, finalgrid, checkpoint, embed, msg, userdata) {
  ...
  var racelength = 0
  for (var i = 0; i < 20; i++) {
    var speed = gte_GTF.lengthAlpha(fppavg, userdata["raceinprogress"]["weatherhistory"][i], racesettings["track"]);
...
module.exports.speedtestresults = function(racelength, racesettings, racedetails, finalgrid, checkpoint, embed, msg, userdata) {
  ...
  gte_STATS.addPlayTime(racelength, userdata);
...
module.exports.careerdifficultycalc = function(difficulty, weather, racesettings) {
  ...
  var fpp = gte_PERF.perfEnthu(gtfcar, "GARAGE")["fpp"];
  /*
    var tires = gte_PARTS.find({ name: gtfcar["perf"]["tires"]["current"], type: "tires" })[0]
...
module.exports.driftsection = function(racesettings, racedetails, finalgrid, checkpoint, embed, msg, userdata, last) {
  ...
  var tire = gte_PARTS.find({ name: racesettings["driver"]["car"]["perf"]["tires"]["current"], type: "tires" })[0]["name"];
...
module.exports.onlinedifficultycalc = function(player, racesettings) {
  var num = 0
  var tires = gte_PARTS.find({ name: player["tires"], type: "tires" })[0]
...
module.exports.licensecheck = function(racesettings, racedetails, finalgrid, embed, msg, userdata) {
  ...
  var bronzecomplete = gte_STATS.checkLicenseTests(option, "3rd", userdata);
  var goldcomplete = gte_STATS.checkLicenseTests(option, "1st", userdata);
  if (bronzecomplete && !gte_STATS.checklicense(option, "", msg, userdata)) {
    setTimeout(function() { gte_STATS.setLicense(option.toUpperCase(), userdata) }, 5000)
    ...
    gte_STATS.redeemGift("🎉 License " + option.toUpperCase() + " Achieved 🎉", prize, embed, msg, userdata);
  }
  if (goldcomplete && gte_STATS.raceEventStatus(licenses[0], userdata) != "✅") {
    var option = option.toLowerCase()
    var total = 6
    if (option.includes("ic")) {
      total = 4
...
module.exports.timetrialracelength = function(racesettings, racedetails, finalgrid, checkpoint, difficulty, embed, msg, userdata) {
  ...
  var speed = gte_GTF.lengthAlpha(fppavg, racesettings["weather"], racesettings["track"]);
...
var value = (racelength * 0.94) + ((racelength * 0.94) * (jstat.gamma.mean(11, beta)/10))
  ...
  */
  ...
  var beta = 0.5 - (0.2 * ((gte_STATS.level(userdata) / 50)))
...
module.exports.timetrialresults = function(racesettings, racedetails, finalgrid, checkpoint, embed, msg, userdata) {
  ...
  gte_STATS.addMileage(mileage[0], userdata);
  gte_STATS.addTotalMileage(mileage[0], userdata);
...
module.exports.createRaceButtons = function(racesettings, racedetails, finalgrid, checkpoint, results2, buttons, emojilist, embed, msg, userdata) {
  ...
  function continuechampionship() {
    ...
    if (typeof trackname !== 'string') {
      ...
      racesettings["track"] = gte_COURSEMAKER.displayCourse(t, bcallback)
    ...
    function continuenextrace() {
      racesettings["positions"] = gte_RACE.creditsCalc(racesettings, "")
      ...
      msg.edit(msgjson).then(msg => {
        gte_RACES2.startSession(racesettings, racedetails, finalgrid, false, embed, msg, userdata);
      })
    }
  }
  function continuedr() {
...
module.exports.updateGrid = function(racesettings, racedetails, finalgrid, checkpoint, timeinterval, number, message, embed, msg, userdata) {
  ...
  if (racesettings["mode"] == "CAREER") {
    difficulty = gte_RACEEX.careerdifficultycalc(difficulty, weather, racesettings)[0]
  }
  ...
  for (var i = 0; i < finalgrid.length; i++) {
    ...
    if (racesettings["mode"] == "ONLINE") {
      ...
      boost = boost + (0.2 * gte_RACEEX.careerdifficultycalc(finalgrid[i], racesettings)[0])
    }
    var jstat = require("jstat")
    if (finalgrid[i]["user"]) {
