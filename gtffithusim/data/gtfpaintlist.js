...
module.exports.list = function (args) {
  var paint = gte_LISTS.gtfpaintslist;
  var results = "";
  if (args.length == 0) {
    return results;
  }
...
module.exports.find = function (args) {
  ...
  var paint = gte_LISTS.gtfpaintlist;
  var final = [];
...
module.exports.applyPaint = function (paint, userdata) {
  if (paint["type"] == "Livery") {
     var installedpart = userdata["garage"][gte_STATS.currentCarNum(userdata) - 1]["livery"];
  } else {
  var installedpart = userdata["garage"][gte_STATS.currentCarNum(userdata) - 1]["color"];
  }
...
if (paint["type"] == "Livery") {
      userdata["garage"][gte_STATS.currentCarNum(userdata) - 1]["livery"] = installedpart
  } else {
   userdata["garage"][gte_STATS.currentCarNum(userdata) - 1]["color"] = installedpart
  }
