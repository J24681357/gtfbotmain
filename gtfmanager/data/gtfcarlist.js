...
module.exports.list = function (args) {
  var gtfcars = gtm_LISTS.gtfcarlist;
  var results = "";
  if (args.length == 0) {
    return results;
  }
  ...
  if (args == "countries") {
    ...
    results = gtm_TOOLS.unique(results)
  }
  if (args == "drivetrains") {
    ...
    for (var i = 0; i < makekey.length; i++) {
      ...
      if (args["upperfpp"] !== undefined) {
        if (args["upperfpp"].length == 0) {
          ...
          var x = gtm_PERF.perf(makekey[i], "DEALERSHIP")["fpp"];
          if (x <= upperfpp) {
            count++;
          }
        }
  ...
  final.sort(function (a, b) {
    ...
    return a["name"].toString().localeCompare(b["name"].toString());
      ...
      if (sort == "hpasc" || sort == "Lowest Power") {
        return gtm_PERF.perf(a, "DEALERSHIP")["power"] - gtm_PERF.perf(b, "DEALERSHIP")["power"];
      } else if (sort == "hpdesc" || sort == "Highest Power") {
        return gtm_PERF.perf(b, "DEALERSHIP")["power"] - gtm_PERF.perf(a, "DEALERSHIP")["power"];
      } else if (sort == "weightasc" || sort == "Lowest Weight") {
        return gtm_PERF.perf(a, "DEALERSHIP")["weight"] - gtm_PERF.perf(b, "DEALERSHIP")["weight"];
      } else if (sort == "weightdesc" || sort == "Highest Weight") {
        return gtm_PERF.perf(b, "DEALERSHIP")["weight"] - gtm_PERF.perf(a, "DEALERSHIP")["weight"];
      } else if (sort == "fppasc" || sort == "Lowest FPP") {
        return gtm_PERF.perf(a, "DEALERSHIP")["fpp"] - gtm_PERF.perf(b, "DEALERSHIP")["fpp"];
...
module.exports.get = function (args) {
  ...
  var makelist = gtm_LISTS.gtfcarlist[make]
...
module.exports.addCar = function (car, arg, userdata) {
  ...
  if (arg != "LOAN") {
    if (gtm_STATS.garage(userdata).length == 0) {
      gtm_STATS.setCurrentCar(1, undefined, userdata);
      userdata["currentcar"]++;
    }
  }
  car["condition"] = 100
  ...
  var fpp = gtm_PERF.perf(car, "DEALERSHIP")["fpp"];
  var sell = gtm_GTFAUTO.sellCalc(car);
  if (arg != "LOAN") {
  ...
  newcar["fpp"] = gtm_PERF.perf(newcar, "GARAGE")["fpp"];
  ...
  if (arg == "ITEM" || arg == "LOAN") {
    ...
    if (arg == "SORT") {
      userdata["garage"] = gtm_STATS.sortGarage(userdata);
    }
    gtm_STATS.save(userdata);
    return;
  }
};
...
module.exports.audit = async function () {
  ...
  for (var make = 0; make < makes.length; make++) {
    ...
    for (var i = 0; i < gtfcars[makes[make]].length; i++) { 
      ...
      car["_id"] = index
      var perf = gtm_PERF.perf(car, "DEALERSHIP");
