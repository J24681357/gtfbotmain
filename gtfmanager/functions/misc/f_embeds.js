...
module.exports.alert = function (object, msg, userdata) {
  ...
  if (name.includes("✅") || name.includes("🎉") || name.includes(gtm_EMOTE.goldmedal)) {
    color = 0x216c2a;
  }

  var message = msg.content.split("***").join(" ");
  if (message.length == 0 || name.includes("✅") || name.includes("🎉") || name.includes(gtm_EMOTE.goldmedal)) {
    message = "";
  } else {
    message = ' "' + message + '"'
  }
...
embed.setAuthor({name: msg.guild.members.cache.get(userdata["id"]).user.displayName, iconURL: msg.guild.members.cache.get(userdata["id"]).user.displayAvatarURL()});
    ...
    if (["✅", "🎉", gtm_EMOTE.goldmedal].indexOf(name) + 1) {
 ...
 if (msg.type == 20 || msg.type == 0) {
      ...
      if (seconds > 0) {
        gtm_STATS.addCount(userdata)
          gtm_DISCORD.delete(msg, {seconds: 5}, function() {
  ...
  return msg.edit({ embeds: [embed] }).then(msg => {
      if (seconds > 0) {
        setTimeout(() => {
          gtm_DISCORD.delete(msg, {})
          gtm_MAIN.embedcounts[userdata["id"]]--;
          }, seconds * 1000)
      }
    });
  }
