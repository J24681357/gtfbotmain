  ...
  for (var key = 0; key < parttypes.length; key++) {
    ...
    for (var i = 0; i < typekey.length; i++) {
      ...
      if (args["engine"] !== undefined) {
        if (args["engine"].length == 0) {
          count++;
        } else {
          var engine = args["engine"];

          if (typekey[i]["engines"].length == 0) {
            count++
          }
          for (var iengine = 0; iengine < typekey[i]["engines"].length; iengine++) {
            if (engine.includes(typekey[i]["engines"][iengine])) {
              count++;
              break;
            }
          }
      ...
      if (args["special"] !== undefined) {
        if (args["special"].length == 0) {
          ...
          for (var ispecial = 0; ispecial < specials.length; ispecial++) {
            if (typekey[i]["special"] == specials[ispecial]) {
              count = total
            }
          }
          if (count != total) {
...
module.exports.tuningList = function(part, gtfcar, embed, msg, userdata) {
  ...
  if (part["type"] == "Ballast Weight") {
      ...
      [
        ...
        function(x) {
          if (gtf_STATS.weightUnits(userdata) == "kg") {
          return x * 20
          } else {
            return Math.round(x * 44.0925)
          }
        }
      ],
    ];
  ...
  if (part["type"] == "Ballast Weight") {
     var tunevalues = gtfcar["perf"]["items"].filter(x => x["name"] == part["type"])[0]["tuning"];
  } else {
...
module.exports.checkParts = function(part, gtfcar) {
  var preview = gtf_PARTS.previewPart(part, gtfcar, "GARAGE")
  var fpp = preview["fpp"].toString()
  var items = gtfcar["perf"]["items"]

  var type = part["type"].toLowerCase().replace(/ /g, "")

  
  if (type == "ballastweight") {
    for (var i = 0; i < items.length; i++) {
      if (gtfcar["perf"]["items"][i]["name"] == "Ballast Weight") {
        return ["✅", "**" + gtfcar["fpp"] + "**"]
      }
    }
    return ["", "**" + fpp + "**"]
...
module.exports.previewPart = function(part, car, condition) {
  ...
  var type = part["type"].toLowerCase().replace(/ /g, "")
  if (type == "ballastweight") {
    var items = car2["perf"]["items"]
      if (items.length == 0) {
        return gtf_PERF.perf(car2, condition);
      } else {
        var index = car2["perf"]["items"].findIndex(x => x["name"] == part["type"])
        car2["perf"]["items"][index]["tuning"] = part["tuning"];
        return gtf_PERF.perf(car2, condition);
      } 
  }
...
module.exports.installPart = function(part, userdata) {
  ...
  if (type == "ballastweight") {
    userdata["garage"][gtf_STATS.currentCarNum(userdata) - 1]["perf"]["items"].push({ "name": "Ballast Weight", "tuning": [0, 0, 0] })
    return
  }
...
module.exports.costCalc = function(part, gtfcar, ocar) {
  if (part["type"] == "Tires" || part["type"] == "Car Wash" || part["type"] == "Ballast Weight") {
    return part["cost"]
  }
