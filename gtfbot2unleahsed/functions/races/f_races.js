      ...
      msg.edit({ embeds: [embed], components: buttons }).then(msg => {
        ...
        function flagstartrace() {
          if (userdata["raceinprogress"]["active"]) {
            require((__dirname.split("/").slice(0, 4).join("/") + "/").includes("gtfmanager") || (__dirname.split("/").slice(0, 4).join("/") + "/").includes("gtfbot2unleahsed") ? __dirname.split("/").slice(0, 4).join("/") + "/" : __dirname.split("/").slice(0, 5).join("/") + "/" + "commands/status").execute(msg, { options: "exit" }, userdata);
          } else {
    ...
    function nextfunc(msg) {
      ...
      if (event["eventid"].includes("SEASONAL")) {
        functionlist.push(function() {
          require((__dirname.split("/").slice(0, 4).join("/") + "/").includes("gtfmanager") || (__dirname.split("/").slice(0, 4).join("/") + "/").includes("gtfbot2unleahsed") ? __dirname.split("/").slice(0, 4).join("/") + "/" : __dirname.split("/").slice(0, 5).join("/") + "/" + "commands/seasonal").execute(msg, { options: event["eventid"].split("SEASONAL")[1].split("-")[0] }, userdata);
        })
      } else {
        functionlist.push(function() {
          require((__dirname.split("/").slice(0, 4).join("/") + "/").includes("gtfmanager") || (__dirname.split("/").slice(0, 4).join("/") + "/").includes("gtfbot2unleahsed") ? __dirname.split("/").slice(0, 4).join("/") + "/" : __dirname.split("/").slice(0, 5).join("/") + "/" + "commands/career").execute(msg, { options: event["eventid"].split("-")[0] }, userdata);
        })
      }
      gtf_TOOLS.createButtons(buttons, emojilist, functionlist, msg, userdata)
    }
